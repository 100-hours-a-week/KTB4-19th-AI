...
파싱 → 정제 → 마스킹 → 청킹 → BGE-M3 실인코딩 → 적재 → 격리·교체·정리·인덱스 확인.
...
def request(doc_id: str = "doc-e2e") -> IndexingJobRequest:
    return IndexingJobRequest(
        building_id=101,
        doc_id=doc_id,
        title="e2e 스모크",
        file_key="s3://zipsai-files/documents/e2e.pdf",
    )
...
def main() -> None:
    ...
    upsert_document(client, request("doc-stale"), embedded[:1], collection=COLLECTION)
    delete_missing_documents(client, 101, ["doc-e2e"], collection=COLLECTION)
    remaining = client.count(collection_name=COLLECTION, exact=True).count
    assert remaining == 1, remaining
    print("PASS 정리: 유효 목록에 없는 doc-stale만 삭제 — 총 1건")
